...
public class Belt {
	...
	public synchronized void put(Bicycle bicycle, int index) 
		...
		System.out.println(bicycle + " arrived < " + beltName + " >");
	...
	public synchronized Bicycle getEndBelt() throws InterruptedException {
		...
		// make a note of the event in output trace
		System.out.print(indentation + indentation + indentation + indentation);
		System.out.println(bicycle + " departed < " + beltName + " >");
	...
	OverloadException {
		...
		for (int i = segment.length - 1; i > 0; i--) {
			if (this.segment[i - 1] != null) {
				System.out.println(
						indentation + indentation + "< "+beltName + " > | " + 
				this.segment[i - 1] + " [ s" + (i) + " -> s" + (i + 1) + " ]");
			}
